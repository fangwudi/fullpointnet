...
class Eval(object):
    ...
    def evaluate(self, gtAll, dtAll):
        ...
        num_d = len(dtAll)
        tic = time.time()
        print('Running keypoint evaluation: gt {} images; '
              'dt {} images'.format(num_image, num_d))
        sum_match = np.zeros(len(self.iouThrs), dtype=int)
        sum_object = 0
        for x in range(num_image):
            num_match, num_object = self.evaluateImage(gtAll[x], dtAll[x])
            sum_match += num_match
            sum_object += num_object
        if sum_object <= 0:
            print("Wanrning: ground truth object 0 in all images!")
    ...
    def evaluateImage(self, gts, dts):
        ...
        if G == 0:
            ...
            print("Wanrning: no ground truth object in one image")
            return num_match, 0
        if D == 0:
            # no detection object but have groud truth
            print("Wanrning: no detection object in one image")
            return num_match, D
        ...
        for j, gt in enumerate(gts):
            ...
            yg = g[1::3]
            vg = np.array(g[2::3])
            # see if one object has 0 keypoint
        ...
        # init match array
        gtm = np.zeros((T, G)) - 1  # use -1 to init
        dtm = np.zeros((T, D)) - 1
        # find match
        for tind, t in enumerate(self.iouThrs):
            for dind in range(D):
                ...
                gtm[tind, m] = dind
        # count
        num_match = np.count_nonzero((dtm > -1), axis=1)
        return num_match, D
