...
# -*- coding: utf-8 -*-
"""
__title__ = "DepthwiseConv2D"
__author__ = 'fangwudi'
__time__ = '17-12-7 11：34'
...
class DepthwiseConv2D(Conv2D):
    ...
    def build(self, input_shape):
        ...
        # fix output filters
        self.filters = self.depth_multiplier * input_dim
        if self.use_bias:
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.data_format not in {'channels_first', 'channels_last'}:
            raise ValueError('Unknown data_format ' + str(self.data_format))

        x = _preprocess_conv2d_input(inputs, self.data_format)
        padding = _preprocess_padding(self.padding)
        strides = (1,) + self.strides + (1,)
        # add x in function
        outputs = tf.nn.depthwise_conv2d(x, self.depthwise_kernel,
                                         strides=strides,
                                         padding=padding,
